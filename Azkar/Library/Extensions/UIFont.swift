...
extension UIFont {
  ...
  private static var descriptors: [UIFont.TextStyle: UIFontDescriptor] = [:]

  static func textFont(for textStyle: UIFont.TextStyle, maxSize: CGFloat? = nil, type: FontType = .regular) -> UIFont {
    let descriptor = descriptors[textStyle] ?? UIFontDescriptor.preferredFontDescriptor(withTextStyle: textStyle)
